...
- **`portal/models/library.py`'s unconditional `"approved"` status** is correct behavior for human-placed documents per `LIBRARY_INGESTION_RULE.md` — a human placing a document is the approval act, and no Verified/Partially Verified/Unverified/Rejected classification applies to that path. Intelligence Verification classification applies only to cognitively-derived candidates (Publisher drafts, Intelligence nominations, Archive nominations) headed for the Library Promotion Workflow — the remaining build item is adding an `origin` field so those candidates are routed there instead of through the human-ingestion path (§5, Matrix row 8).
...
| 6 | Add origin-gated Library ingestion | `portal/models/library.py::add_record()` | `LIBRARY_INGESTION_RULE.md`; `INTELLIGENCE_VERIFICATION_WORKFLOW.md` §7; Matrix row 8 | Add an `origin` field (`human`/`publisher`/`intelligence`/`archive`); keep immediate `"status": "approved"` for `origin: human`; route non-human origins through a `pending` status + promotion function requiring an Approval Event | Modify | Library ingestion-path tests (human immediate-accept, non-human gated), no-fabrication tests | Mike | High |
...
| 21 | Security sub-library (PIN-gated) | *(none — new)* `portal/models/library.py`, `portal/security/` (from #1–2) | `LIBRARY_INGESTION_RULE.md` §6; Security Spec §4.3, §4.5, §11 | Add a `security` Library section requiring a separate PIN re-check at access time (distinct from session login PIN); wire PIN reset through the existing Security Spec §4.5 workflow | Build New | PIN re-check access tests, PIN reset tests, boundary tests confirming Publisher/Intelligence never read raw contents | Mike | High |
| 22 | Scanner API integration placeholder | *(none — new)* | `LIBRARY_INGESTION_RULE.md` §7 | Document the intended ingestion contract only (a scanner-originated document enters exactly like any human-placed document); no vendor/protocol selection, no implementation | Build New (placeholder only) | None (documentation only) | Not required (placeholder) | Future |
...
- Routing Publisher-generated output through the Library's human-ingestion path (`origin: human`) to bypass the Library Promotion Workflow — the Human Ingestion Rule applies only to documents a human actually places, not to cognitive output relayed through a human's hands.
...
**What should Jules build first?** Security Foundation (Jules items #1–3) — it blocks safe deployment and gates every future Approval Event retrofit — followed by the Library origin-gating fix (#6) and the Security sub-library build (#21), both small, contained additions now that `LIBRARY_INGESTION_RULE.md` has clarified that human ingestion was never itself a doctrine conflict.
