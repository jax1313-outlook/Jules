...
def test_stakeholder_data_isolation(client):
    """Test External Stakeholder API excludes internal proprietary keys/scoring."""
    response = client.get('/api/v1/stakeholder/shipment/L1T-2026-8804')
    assert response.status_code == 200
    data = response.get_json()
    # Confirm expected public fields are present
    assert 'load_number' in data
    assert 'public_route_notice' in data
    # Confirm internal scoring/notes/work items are strictly EXCLUDED
    assert 'work_item_id' not in data
    assert 'cognitive_status' not in data
    assert 'scoring_model' not in data
