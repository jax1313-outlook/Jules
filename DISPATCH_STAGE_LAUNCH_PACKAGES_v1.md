...
| 12. Manager Reconciliation and Build | **Approved & executed (Phases M2+M3)** (see `DISPATCH_BLUEPRINT_DECISION_LOG.md`) | Not part of the original 22-item matrix — Manager has its own build matrix, see `DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md` §21; Phases M4–M7 remain deferred | Yes — `Dispatch` branch `stage12-manager-foundation` |
...
**Status: APPROVED & EXECUTED (Phases M2 + M3), NARROWED SCOPE.** Reconciliation and design (`DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md`) delivered first, analysis-only, retroactively assigned to this stage when Mike instructed "Add a dedicated Manager stage to the 13-stage plan." Mike then approved a build ("Approve Stage 12 build"), and this narrow build design was reviewed and approved ("Approve design"): `DISPATCH_STAGE12_MANAGER_BUILD_DESIGN_v1.md`, scoping the build to Phases M2 (Notification Store) + M3 (Portal Card Preparation) combined — a read-only Staff Report generator over five existing signal sources, surfaced through one new, view-only `/manager` Portal page. Built and delivered to the `Dispatch` repository as `dispatch/manager/` (mirroring `dispatch/spine/` and `dispatch/security/`'s placement) plus `portal/routes/manager.py` and `portal/templates/manager.html`. Zero new database tables, zero Spine schema changes. See `STAGE12_MANAGER_FOUNDATION_WALKTHROUGH_REPORT_v1.md` in the `Dispatch` repository for full verification detail.

**Depends on:** Stage 11 complete (MVP Integration — the reconciliation that surfaced this gap). ✅ Also draws on Stage 4 (Spine — Work Items, transition machinery), Stage 5 (Portal card model), and Stage 7 (Security Foundation — the codebase's dependency-layering lesson, reapplied to `dispatch/manager/`'s placement), all already built. ✅

**Purpose (as built):** Read five already-existing, already-tested signal sources (stalled loads, overdue settlements, open exceptions, unresolved Conflict Notices, IFTA suspect entries); classify each per `MANAGER.md` §7's nine-class taxonomy; rank by the nine-tier priority framework; for anything clearing the Review Needed bar, create a Work Item + Portal Card through the Spine's existing, unmodified transition machinery. Stage Gate, Archive/IFTA, and Security Alert monitors (Phases M4–M6) and any policy-routing hook (Phase M7) are explicitly **not** part of this build — deferred to future, separate, unapproved phases.

**Scope (as built):** New `dispatch/manager/` module (`signals.py`, `classify.py`, `priority.py`, `staff_report.py`); `portal/routes/manager.py` (`GET /manager` only); `portal/templates/manager.html`; one nav link in `base.html`.

**Doctrine source:** `MANAGER.md` in full; `DISPATCH_CONSTITUTION_v3.md` §6–7, §15, §17, §20; `DISPATCH_SPINE_SPECIFICATION_v1.md`; `INTELLIGENCE_VERIFICATION_WORKFLOW.md` (IFTA suspect entries as Partially-Verified-shaped signals).

**Jules Build Matrix items:** None from the original 22-item matrix — Manager was never represented in it. Manager's own build matrix (`DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md` §21) items covered by this build: signal aggregation, Work Item classification, card preparation, priority ranking. Not covered: the `ROUTED_TO_MANAGER` transition-target amendment (found unnecessary — a fully-allowed transition path to `PORTAL_CARD_CREATED` already exists that never touches it), card-level unification, security event pattern detection.

**Findings:** Confirmed directly against the codebase before implementation: nothing in `portal/` rendered anything from the Spine's `portal_cards` table (the concrete form of Stage 11's Jules #9 gap) — resolved narrowly with one new read-only route, not a general Sandbox/Work Item bridge. `dispatch.services.check_overdue_settlements()`, listed as a read-only source in the build design, turned out to mutate state and send an email as a side effect — corrected during implementation to a genuine read, flagged in the walkthrough report. A live-walkthrough-only defect was found and fixed in the same pass: dedup was correctly preventing duplicate Work Item creation but was also suppressing the *display* of already-materialized, still-unresolved cards after their first view — fixed by separating "materialize once" from "display everything still active."

**Deliverables:** A working `GET /manager` page showing a classification-count summary and every currently-active card (ranked, highest priority first), reused across page loads without disappearing or duplicating.

**Test plan (executed):** `tests/test_manager_foundation.py` — 30 tests covering signal aggregation (all five sources, empty state, resolved/closed items correctly excluded), classification thresholds, priority tier ranking (including the "Tier 1 always wins over card_level" rule), dedup (no duplicate Work Items, cards remain visible across repeated passes), Spine interaction (exact transition path, `ROUTED_TO_MANAGER` never touched, fixed required-closing sentence present), structural guards (no direct `current_state` write, no security/approval/booking calls, GET-only route), and Portal rendering. Full regression: 2,432 tests, 0 failures (2,402 baseline + 30 new).

**Walkthrough report:** Delivered — `STAGE12_MANAGER_FOUNDATION_WALKTHROUGH_REPORT_v1.md` in the `Dispatch` repository. Live dev-server run seeding one signal per source, confirming correct classification/ranking/rendering, the dedup-vs-display fix (three consecutive page loads, all four cards remained visible, no duplicates), and that `/settings`, `/home`, and every existing route remain exactly as Stage 7 left them.

**Stop/Go:** **Go, for the narrowed scope (Phases M2+M3).** Phases M4–M7 remain deferred, unapproved future work, per the build design's explicit out-of-scope section.
