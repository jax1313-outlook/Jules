...
**Status: APPROVED & EXECUTED. All seven phases (M1–M7) of the Manager build-out design are now delivered.** Reconciliation and design (`DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md`) delivered first, analysis-only, retroactively assigned to this stage when Mike instructed "Add a dedicated Manager stage to the 13-stage plan." Built in five approved passes:
...
- **Pass 3 (M4):** "Design the dispatch/docs/ mirror approach for M4" → `DISPATCH_STAGE12_MANAGER_M4_MIRROR_DESIGN_v1.md` → "Approve design" → a hand-authored, structured `docs/STAGE_STATUS.json` mirror (extending Stage 2's existing `docs/` mirror) plus `dispatch/manager/stage_gate.py`, surfaced as a separate, fail-soft Stage Gate panel on `/manager`. See `STAGE12_MANAGER_M4_WALKTHROUGH_REPORT_v1.md`.
- **Pass 4 (M5 Archive half, completing M5):** "Wire Manager to the new Archive Review Queue" → `DISPATCH_STAGE12_MANAGER_ARCHIVE_WIRING_DESIGN_v1.md` → "Approve design" → an eighth signal source reading Stage 6's Archive Review Queue directly; corrected a previously-latent card-level bug (`ARCHIVE` mapped below the review bar since Pass 1, never reachable until now) along the way. See `STAGE12_MANAGER_ARCHIVE_WIRING_WALKTHROUGH_REPORT_v1.md`.
- **Pass 5 (M7, completing the design's full scope):** "Approve Stage 12 build for M7" → `DISPATCH_STAGE12_MANAGER_M7_POLICY_HOOK_DESIGN_v1.md` → "Approve design" → a strictly read-only `GET /api/manager/policy-candidates`, exposing only Auto-log-tier (card_level 0) counts — explicitly **not** a policy engine, does not design or implement "GX," has no consumer anywhere in this codebase. See `STAGE12_MANAGER_M7_POLICY_HOOK_WALKTHROUGH_REPORT_v1.md`.

All five passes built and delivered to the `Dispatch` repository as `dispatch/manager/` (mirroring `dispatch/spine/` and `dispatch/security/`'s placement) plus `portal/routes/manager.py` and `portal/templates/manager.html`. Zero new database tables, zero Spine schema changes, across all five passes.

**Depends on:** Stage 11 complete (MVP Integration — the reconciliation that surfaced this gap). ✅ Also draws on Stage 4 (Spine — Work Items, transition machinery), Stage 5 (Portal card model), Stage 7 (Security Foundation — both the dependency-layering lesson reapplied to `dispatch/manager/`'s placement, and `list_security_events()` as M6's read source), Stage 2 (Documentation Import — the `docs/` mirror pattern M4 extends), and Stage 6 (Archive Review Queue — M5's Archive-half read source), all already built. ✅

**Purpose (as built):** Read eight already-existing, already-tested signal sources (stalled loads, overdue settlements, open exceptions, unresolved Conflict Notices, IFTA suspect entries, IFTA exceptions on draft approvals, detected security event patterns, and Archive Review Queue items); classify each per `MANAGER.md` §7's nine-class taxonomy; rank by the nine-tier priority framework; for anything clearing the Review Needed bar, create a Work Item + Portal Card through the Spine's existing, unmodified transition machinery. Separately, surface Migration Plan stage status from a manually-refreshed local mirror, never a live cross-repo integration. Separately again, expose only the lowest-risk (Auto-log-tier) classification counts through a narrow, unconsumed interface candidate — the full extent doctrine ever authorized this stage to go toward "policy routing," with an actual policy engine explicitly named as a distinct future mission outside this stage's scope.

**Scope (as built):** `dispatch/manager/` module (`signals.py`, `classify.py`, `priority.py`, `staff_report.py`, `security_monitor.py`, `stage_gate.py`, `policy_candidates.py`); `docs/STAGE_STATUS.json` (new mirror file, refreshed again in Pass 4) and `docs/README.md` (updated); `portal/routes/manager.py` (`GET /manager`, `GET /api/manager/policy-candidates`); `portal/templates/manager.html`; one nav link in `base.html`. `staff_report.py` required zero changes across all five passes — the orchestrator is genuinely source-type-agnostic, confirmed for a ninth time in Pass 5, and both Stage Gate status and the M7 policy-candidate summary were deliberately kept out of that pipeline (neither is a discrete, dedup-needing event).

**Doctrine source:** `MANAGER.md` in full; `DISPATCH_CONSTITUTION_v3.md` §6–7, §15, §17, §20; `DISPATCH_SPINE_SPECIFICATION_v1.md`; `INTELLIGENCE_VERIFICATION_WORKFLOW.md`; `SECURITY_AND_AUTHENTICATION_SPECIFICATION_v1.md` §12.1 (Manager's read-only security boundary); `ARCHIVE_REVIEW_POLICY.md` (Pass 4's read source's own doctrine); `DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md` §18 (Manager Policy Engine Relationship — Pass 5's governing boundary).

**Jules Build Matrix items:** None from the original 22-item matrix — Manager was never represented in it. Manager's own build matrix (`DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md` §21) items covered across all five passes: signal aggregation, Work Item classification, card preparation, priority ranking, security event pattern detection, Stage Gate status surfacing, Archive Review Queue surfacing, policy routing hook candidate. Not covered: the `ROUTED_TO_MANAGER` transition-target amendment (found unnecessary — a fully-allowed transition path to `PORTAL_CARD_CREATED` already exists that never touches it), card-level unification (the three independent `PortalCard.card_level`/Sandbox/Conflict implementations — a pre-existing Conflict, still unresolved). Neither of these was ever part of Manager's own seven-phase scope.

**Findings:** Confirmed directly against the codebase before Pass 1: nothing in `portal/` rendered anything from the Spine's `portal_cards` table (the concrete form of Stage 11's Jules #9 gap) — resolved narrowly with one new read-only route. `dispatch.services.check_overdue_settlements()`, listed as read-only in the Pass 1 design, turned out to mutate state and send an email as a side effect — corrected to a genuine read. A live-walkthrough-only defect in Pass 1 (dedup was suppressing display of still-unresolved cards after their first view) was found and fixed in the same pass. Before Pass 2: confirmed M4 has no cross-repo mechanism to build on, and M5's Archive half is still blocked — both flagged rather than forced. Pass 2's live walkthrough confirmed dedup holds correctly even as the underlying security-event count kept growing (5 total failures, still exactly 1 card), and that a sealed IFTA approval's exceptions correctly stop appearing in fresh signal collection while its already-materialized card knowingly persists (a flagged, accepted limitation, not a defect). Before Pass 3: confirmed the correct mirror location is `docs/` at the repo root, not `dispatch/docs/` as an earlier document had loosely stated — corrected before implementation. Pass 3's live walkthrough proved the fail-soft contract directly: `docs/STAGE_STATUS.json` was moved aside mid-walkthrough, `/manager` still rendered 200 with the signal pipeline fully intact and only the Stage Gate panel absent, then the file was restored. Before Pass 4: found `classify.py`'s `ARCHIVE` class mapped to card level 1 — below the review bar of 2 — since Pass 1, dead code until this pass; corrected to 2, matching the buildout design's own Portal Card Model table. Pass 4's live walkthrough resolved a seeded item through the *real* Stage 6 `/archive` Keep/Delete route and confirmed Manager's next signal collection correctly stopped detecting it, while also surfacing (and then fixing) a stale `docs/STAGE_STATUS.json` recommendation still pointing at Stage 6 after it had already shipped. Pass 5's own governing constraint — restated verbatim in its design document before any code was proposed — was the original Manager mission's "Do not design GX, do not implement GX"; the delivered scope was deliberately narrowed to counts-only, Auto-log-tier-only, with an explicit statement that no future work under this stage's name may build an actual policy-engine consumer without its own separate design cycle.

**Deliverables:** A working `GET /manager` page showing a classification-count summary and every currently-active card (ranked, highest priority first) across all eight signal sources, reused across page loads without disappearing or duplicating, plus a separate Stage Gate Status panel mirroring the Migration Plan's current state (absent without error when its source file is unavailable), plus a separate, unconsumed `GET /api/manager/policy-candidates` interface candidate.

**Test plan (executed):** `tests/test_manager_foundation.py` — 66 tests total (30 from Pass 1 + 12 from Pass 2 + 8 from Pass 3 + 8 from Pass 4 + 8 from Pass 5) covering signal aggregation, classification thresholds, priority tier ranking, dedup (including cross-day pattern renewal), Spine interaction, structural guards (no direct `current_state` write, no security-write calls anywhere, no `docs/`-write calls, no `mark_reviewed()` call, no approval/booking/submission calls anywhere including the new M7 module, GET-only routes), Portal rendering, the Stage Gate mirror's fail-soft contract, a direct regression test for the `ARCHIVE` card-level fix, and confirmation that the M7 endpoint never exposes individual record data or any classification above Auto-log tier. Full regression: 2,489 tests, 0 failures (2,481 baseline + 8 new).

**Walkthrough reports:** All five delivered in the `Dispatch` repository — `STAGE12_MANAGER_FOUNDATION_WALKTHROUGH_REPORT_v1.md` (Pass 1), `STAGE12_MANAGER_M4_M6_WALKTHROUGH_REPORT_v1.md` (Pass 2), `STAGE12_MANAGER_M4_WALKTHROUGH_REPORT_v1.md` (Pass 3), `STAGE12_MANAGER_ARCHIVE_WIRING_WALKTHROUGH_REPORT_v1.md` (Pass 4), `STAGE12_MANAGER_M7_POLICY_HOOK_WALKTHROUGH_REPORT_v1.md` (Pass 5).

**Stop/Go:** **Go — Stage 12 is complete.** All seven phases of `DISPATCH_MANAGER_BUILDOUT_DESIGN_v1.md`'s original M1–M7 scope are delivered. No further Manager build work is authorized or expected under this stage; any future policy-engine consumer of Pass 5's interface candidate is explicitly named as a separate future mission requiring its own full design-and-approval cycle, not an extension of this stage.
